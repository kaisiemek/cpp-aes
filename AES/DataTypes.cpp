//
// Created by Kai Siemek on 20.12.21.
//

#include <iomanip>
#include "DataTypes.h"


AES::byte AES::gf_256_mult(AES::byte x, AES::byte y)
{
  // refer to https://www.samiam.org/galois.html
  static constexpr std::array<uint8_t, 256> galois_log_table {
    ,
  };

  // refer to https://www.samiam.org/galois.html
  static constexpr std::array<uint8_t, 256> galois_exp_table {
    ,
  };
  // Anything multiplied with 0 is 0, even in GF256 arithmetic :)
  if (x == 0 || y == 0) {
    return 0x00;
  }

  // Fancy calculations with log and exp tables, for more info on
  // the maths behind this refer to https://www.samiam.org/galois.html
  auto res {(galois_log_table[x] + galois_log_table[y]) % 255};
  return galois_exp_table[res];
}

std::ostream &operator<<(std::ostream &os, const AES::block &x)
{
  for (int i {0}; i < x.size(); ++i) {
    os << std::hex << std::setfill('0') << std::setw(2) << +x[i];
    if ((i + 1) % 4 == 0 && i != 0)
      os << ' ';
  }

  return os;
}
