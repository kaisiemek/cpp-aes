...
#ifndef AES_CPP_CONSTANTS_H
#define AES_CPP_CONSTANTS_H

#include <array>
#include <bit>

namespace AES {
  // Bitcast to byte, byte array does not support constant initialisation without casting
  // each literal to a byte first (very ugly)
  constexpr auto s_box = std::bit_cast<std::array<std::byte, 256>>(
  ...
  constexpr auto inverse_s_box = std::bit_cast<std::array<std::byte, 256>>(
    std::array<uint8_t, 256> {
  ...
  constexpr std::array<uint16_t, 256> galois_log_table {
  ...
  constexpr auto galois_exp_table = std::bit_cast<std::array<std::byte, 256>>(
    std::array<uint8_t, 256> {
  ...
  constexpr int AES128_ENCRYPTION_ROUNDS = 10;
}

#endif //AES_CPP_CONSTANTS_H
